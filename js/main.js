$(window).bind("load", function() {
    ...
    steem.api.setOptions({ url: steem_rpc_nodes[0] });
    ...
    async function getSurfHistory () {
        try
        {
            ...
            const historyRaw = await callGetSteemSurfHistory("helios.voter"); 
            ...
            history.forEach((tx) => {            
                ...
                const html = `
                    <tr class="table-font">
                        <td><a class="link-info" href="https://steemit.com/@${author}" target="_blank">@${author}</a></td>
                        <td><a class="link-info" href="https://steemit.com/@${author}/${permlink}" target="_blank">${permlinkShort}...</a></td>
                        <td>${weightPercent}%</td>
                        <td>${dateString} ${timeString}</td>
                        <td><a class="link-info" href="https://steemblockexplorer.com/tx/${trx_id}" target="_blank">${txIdShort}...</a>
    ...
    async function callGetSteemSurfHistory(author) {
        return new Promise((resolve, reject) => {
            steem.api.getAccountHistory(author, -1, 100, (err, result) => {
                if (err) 
                {
                    reject(err);
                } 
    ...
    */   
    ...
    async function postURL (post_link) {
        ...
        {    
            ...
            var postData = await callGetSteemContent(author, link);                      
    ...
    async function callGetSteemContent(author, permlink) {
        return new Promise((resolve, reject) => {
